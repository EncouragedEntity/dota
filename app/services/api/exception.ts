...
class HttpException<T = any, D = any> extends Error {
  ...
  constructor(message?: string, payload?: HttpError<T, D>) {
    ...
    if (this.type.network) this.message = "Network error";

    if (this.type.timeout) this.message = "Timeout error";

    if (this.status === 502) this.message = "Bad gateway";

    if (this.status === 504) this.message = "Gateway timeout";
  }
}
